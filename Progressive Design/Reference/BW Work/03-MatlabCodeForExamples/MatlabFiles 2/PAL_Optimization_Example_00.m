...
% P107=load(['PAL_Optim_' num2str(design_num) '.mat'])
...
i_plot=i_plot+1;
    toc
%%    % optimize meridian power first
    tic
    delta_y=1;
    PAL=PAL.M_Set_New_delta_y(delta_y);    % this sets the grid delta used for calculating surface, power, cylinder, merit function, etc.
                                            % [xx,yy]=ndgrid(  -ymax:delta_y:ymax )
                                            % ymax=round((PAL.Optimization_Diameter/2+2*delta_y)/delta_y)*delta_y;
 
         
    % here is where the optimization variables are defined  
    %   in CLASS C_PAL  the method M_X0_from_VariableList creates an
    %   array 'x0' which is the input to the optimization function fmincon
    %   (method M_fmincon).  
    %   There is another method M_UpdatePAL_from_x0 which given the array
    %   'x0' from the optimizer will update the PAL using ghe variable
    %   names from variable_list.  
    PAL.FrontSurface.Surface_Description.variable_list={...
         'sphere' 'k'  'add' 'y0' 'NRP_y' 'off_edge_y' ...
         'cx2(2,4)' 'cx2(2,5)' 'cx2(2,6)' 'cx2(2,7)'  'cx2(2,8)'  'cx2(2,9)'  'cx2(2,10)'...  
                };
    PAL=PAL.M_fmincon;    % this is C_PAL method that invokes optimization outine fmincon with merit function and variables defined above.
  %  PAL=PAL.M_GlobalSearch(2);
    PAL.M_plot_all(i_plot+design_num*100,['meridian power optimized ' num2str(i_plot+design_num*100)]);PAL_Saved_Iterations(i_plot)=PAL;
    delete([this_filename '.mat']);
    save_file.PAL_Saved_Iterations=PAL_Saved_Iterations;export_fig(figure(i_plot+design_num*100),pdf_name, '-append'); i_plot=i_plot+1;
    PAL.FrontSurface   

     toc
%% here we have optimiztion of full lens using coefficients describing function   Q1(y)*x^2+ Q2(y)*x^4 + ...
    PAL=PAL.M_Set_New_delta_y(1);
    tic
   
    PAL.FrontSurface.Surface_Description.variable_list={...
      'cx2(1,4)' 'cx2(1,5)' 'cx2(1,6)' 'cx2(1,7)'  'cx2(1,8)'  'cx2(1,9)'  'cx2(1,10)'...  
      'cx2(2,4)' 'cx2(2,5)' 'cx2(2,6)' 'cx2(2,7)'  'cx2(2,8)'  'cx2(2,9)'  'cx2(2,10)'...      
      'cx2(3,4)' 'cx2(3,5)' 'cx2(3,6)' 'cx2(3,7)'  'cx2(3,8)'  'cx2(3,9)'  'cx2(3,10)'...    
      'cx2(4,4)' 'cx2(4,5)' 'cx2(4,6)' 'cx2(4,7)'  'cx2(4,8)'  'cx2(4,9)'  'cx2(4,10)'...    
                };
    PAL=PAL.M_fmincon;  % run optimization
   % PAL=PAL.M_GlobalSearch(2);
    PAL.M_plot_all(i_plot+design_num*100,['full optimize w/ xy polynomials' num2str(i_plot+design_num*100)]);PAL_Saved_Iterations(i_plot)=PAL;
    delete([this_filename '.mat']);
    save_file.PAL_Saved_Iterations=PAL_Saved_Iterations;export_fig(figure(i_plot+design_num*100),pdf_name, '-append'); i_plot=i_plot+1;
    PAL.FrontSurface   
     toc
...
max_optimization_time_seconds=1*60;  
     ...
     wt_cyl_relative_to_power=[1];
        ...
        % be changed
        Adelta_y=     [2 2 2 1 .5  .5];    % delta_y.  this controls he grid spacing used for calculating deritives of sag (to get power/cyl) and also the spacing for calculating the merit function
        Adelta_spline=[5 5 5 5  5   2];    % this is the grid spacing of the spline points
        ASmoothingParameter=[1 1 .25 .25 .25 .25];  % this is a smoothing parameter used on the spline in BW_PAL_SurfaceSAG_V1.  this helps to remove undulations which make power/cyl "lumpy"
        ...
        for ii=1:length(Adelta_y)
            ...
            OptimalityTolerance=AOptimalityTolerance(ii);
             ...
             while PAL1.Optimization_Control.fmincon_output.funcCount>=100000 && (toc(start)<max_optimization_time_seconds)  % default max function calls allowed is 100,000, so if this is (slightly) >100,000 then still converging so this allows it to continue
                 ...
                 % plot and save
                 PAL1.M_plot_all(i_plot+design_num*100,title1,title2);PAL_Saved_Iterations(i_plot)=PAL1;
                 delete([this_filename '.mat']);
                 save_file.PAL_Saved_Iterations=PAL_Saved_Iterations; export_fig(figure(i_plot+design_num*100),pdf_name, '-append'); i_plot=i_plot+1;
