...
The low-curvature approximation treats sag second derivatives as a usable local curvature matrix when slopes are small enough for projected-coordinate errors to be acceptable.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Low Curvature Approximation`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Low Curvature Approximation` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
