...
Base-up prism-thinning can be required in some minus-power or fitting-height cases to balance finished edge thickness.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Base Up Prism Thinning`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Base Up Prism Thinning` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
