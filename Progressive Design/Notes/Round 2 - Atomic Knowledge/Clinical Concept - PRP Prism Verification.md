...
PRP prism verification measures prism at the PAL prism reference point to check intended prism and interocular balance.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `PRP Prism Verification`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `PRP Prism Verification` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
