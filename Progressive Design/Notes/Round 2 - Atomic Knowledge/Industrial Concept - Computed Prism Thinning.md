...
Computed prism-thinning uses prescription, add, fitting height, decentration, and frame shape to calculate a better thinning prism.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Computed Prism Thinning`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Computed Prism Thinning` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
