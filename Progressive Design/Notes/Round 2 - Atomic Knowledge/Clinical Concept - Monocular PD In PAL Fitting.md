...
Monocular PD in PAL fitting places each lens relative to its own pupil center, reducing lateral misalignment in progressive zones.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Monocular PD In PAL Fitting`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Monocular PD In PAL Fitting` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
