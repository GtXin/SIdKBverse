...
Ghost reflections from prism are reflected images displaced more than the refracted image, sometimes made noticeable by prism-thinning.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Ghost Reflections From Prism`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Ghost Reflections From Prism` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
