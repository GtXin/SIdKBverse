...
A cylindrical sag surface has curvature in one principal meridian and zero curvature in the orthogonal meridian, making it the simplest surface model for single-axis cylinder.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Cylindrical Sag Surface`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Cylindrical Sag Surface` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
