...
A PAL surface can be decomposed into a base sphere or conic plus progressive meridian and lateral correction terms.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Base Sphere Plus Progressive Correction`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Base Sphere Plus Progressive Correction` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
