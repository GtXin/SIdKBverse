...
A soft PAL spreads unwanted astigmatism over larger areas to reduce peak error, often narrowing clear zones and increasing required head/eye movement.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Soft PAL Design`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Soft PAL Design` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
