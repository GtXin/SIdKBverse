...
The umbilical line is a corridor idealization where the two principal curvatures are equal, giving zero local surface cylinder along the line.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Umbilical Line`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Umbilical Line` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
