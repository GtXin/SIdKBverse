...
Surface power is derived from the lens surface; user power is derived from the refracted wavefront at the eye in position of use.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Surface Power Versus User Power`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Surface Power Versus User Power` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
