...
Variable-list mapping converts named surface or target parameters into optimizer vector elements and back.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Variable List To Optimizer Vector`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Variable List To Optimizer Vector` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
