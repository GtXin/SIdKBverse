...
Hard versus soft PAL design compares concentrated high astigmatism with wider clear zones against distributed lower astigmatism with narrower clear zones.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Hard Versus Soft PAL Design`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Hard Versus Soft PAL Design` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
