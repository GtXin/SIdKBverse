...
A hard PAL concentrates unwanted astigmatism into smaller areas to preserve wider distance and near zones, often increasing blur/swim in peripheral regions.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Hard PAL Design`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Hard PAL Design` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
