...
Spline sag correction uses an interpolated freeform surface or grid to add flexible local shape changes during optimization.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Spline Sag Correction`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Spline Sag Correction` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
