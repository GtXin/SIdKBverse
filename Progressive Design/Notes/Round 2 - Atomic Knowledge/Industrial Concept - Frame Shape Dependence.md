...
Frame shape dependence means final lens thickness and usable zones depend on the actual cutout, not just the uncut blank.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Frame Shape Dependence`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Frame Shape Dependence` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
