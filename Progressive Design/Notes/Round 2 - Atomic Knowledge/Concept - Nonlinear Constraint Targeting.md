...
Nonlinear constraint targeting enforces limits or max/min conditions directly rather than only penalizing errors in a weighted objective.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Nonlinear Constraint Targeting`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Nonlinear Constraint Targeting` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
