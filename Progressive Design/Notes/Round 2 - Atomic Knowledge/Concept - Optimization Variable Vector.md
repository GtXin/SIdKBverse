...
An optimization variable vector is the numeric parameter vector passed to an optimizer after selected design parameters are packed from named fields.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Optimization Variable Vector`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Optimization Variable Vector` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
