...
A progressive addition lens provides a continuous change from distance correction to near addition without a visible segment, using a spatially varying optical surface.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Progressive Addition Lens`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Progressive Addition Lens` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
