...
Rule-of-thumb prism-thinning often uses a simple add-based formula such as prism = 0.6 x add.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Rule Of Thumb Prism Thinning`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Rule Of Thumb Prism Thinning` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
