...
The intermediate zone is the corridor region that transitions power from distance toward near and controls midrange vision.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Intermediate Zone`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Intermediate Zone` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
