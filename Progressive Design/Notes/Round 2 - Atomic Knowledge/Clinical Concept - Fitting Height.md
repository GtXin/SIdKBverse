...
Fitting height is the vertical placement of the PAL fitting point relative to the frame and pupil, affecting zone access and thickness.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Fitting Height`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Fitting Height` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
