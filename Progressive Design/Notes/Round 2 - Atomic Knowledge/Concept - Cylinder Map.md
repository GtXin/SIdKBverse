...
A cylinder map is a spatial map of local surface astigmatism magnitude or cylinder error derived from principal-curvature splitting.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Cylinder Map`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Cylinder Map` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
