...
Prescription-specific PAL design changes the PAL design by distance Rx or base curve, not only by add.
...
During Ben-code review, search for names, comments, plots, target functions, or class methods that correspond to `Prescription Specific PAL Design`. Add exact file/function references here after the code is read.

## Meta

### File Purpose

This Round 2 atomic note defines `Prescription Specific PAL Design` as a reusable Obsidian knowledge point for progressive lens design. Use it as an independent reference during Ben-code inspection, clinical/application reasoning, or industrial/manufacturing review.
